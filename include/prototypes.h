...
extern uint32 remove_frm_fifo();
extern syscall free_frm(uint32 i);
extern syscall insert_frm_fifo(int frame_num);
extern void update_frm_fifo(uint32 i);
extern uint32 get_faults();
extern syscall init_check();

extern pt_t* create_a_page_table(pid32 pid, int frame_no, int type);
extern pd_t* create_page_dir(pid32 pid);
extern syscall init_global_pt();
extern syscall init_bsm();
extern syscall init_frm();
extern uint32 get_frm_GCA();

extern syscall bsm_map(pid32 pid, int vpno, int source, int npages);
extern syscall bsm_lookup(pid32 pid, long vaddr, bsd_t* store, int* pageth);
